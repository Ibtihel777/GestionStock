...
function MouvementStockList({ onMovementRecorded }) {
  ...
  return (
    <section className="entity-section" aria-labelledby="mouvements-title">
      ...
      {!loading && !error && <div className="table-wrapper"><table><thead><tr><th>Date</th><th>Type</th><th>Article</th><th>Source</th><th>Destination</th><th>Quantité</th></tr></thead><tbody>{mouvements.length === 0 ? <tr><td colSpan="6" className="empty-cell">Aucun mouvement enregistré.</td></tr> : mouvements.map((mouvement) => <tr key={mouvement.id}><td>{new Date(mouvement.dateMouvement).toLocaleString('fr-FR')}</td><td><span className={`movement-badge movement-${mouvement.type}`}>{labels[mouvement.type]}</span></td><td>{mouvement.articleReference}</td><td>{mouvement.codeEmplacementSource ?? '—'}</td><td>{mouvement.codeEmplacementDestination ?? '—'}</td><td>{mouvement.quantite}</td></tr>)}</tbody></table></div>}
    </section>
  );
}
