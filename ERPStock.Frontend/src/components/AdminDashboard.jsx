    ...
    <section className="admin-dashboard" aria-labelledby="dashboard-title">
      ...
      {!loading && !error && <>
        ...
        <div className="dashboard-chart-grid dashboard-chart-grid-wide">
          ...
          <article className="dashboard-panel"><div className="panel-title"><div><p className="dashboard-kicker">Coûts</p><h3>Évolution du coût d’entrée</h3></div></div><label className="dashboard-select">Article<select value={selectedArticleId} onChange={(event) => setSelectedArticleId(event.target.value)}>{data.articles.map((article) => <option key={article.id} value={article.id}>{article.reference} — {article.designation}</option>)}</select></label><LineChart series={[{ label: summary.selectedArticle?.reference ?? 'Article', values: summary.cmupProxy.map((item) => item.value), color: COLORS[3] }]} labels={summary.cmupProxy.map((item) => item.label)} emptyLabel="Aucune entrée enregistrée pour cet article." /><p className="chart-helper">Le prix unitaire des entrées sert de proxy : l’historique exact du CMUP n’est pas encore enregistré.</p></article>
        </div>

        <div className="dashboard-chart-grid">
          <article className="dashboard-panel"><div className="panel-title"><div><p className="dashboard-kicker">Fiabilité</p><h3>Signalements par mois</h3></div></div><div className="stacked-bars">{summary.alertsByMonth.map((month) => { const total = month.pending + month.treated; return <div key={month.key}><strong>{total || ''}</strong><i><b className="treated" style={{ height: `${total ? (month.treated / total) * 100 : 0}%` }} /><b className="pending" style={{ height: `${total ? (month.pending / total) * 100 : 0}%` }} /></i><span>{month.label}</span></div>; })}</div><div className="line-chart-legend"><span><i className="legend-treated" />Traités</span><span><i className="legend-pending" />En attente</span></div></article>
          <article className="dashboard-panel"><div className="panel-title"><div><p className="dashboard-kicker">Alertes</p><h3>Derniers écarts à traiter</h3></div><button type="button" className="text-button" onClick={onOpenManagement}>Voir les alertes</button></div>{summary.recentAlerts.length === 0 ? <p className="empty-dashboard">Aucune alerte en attente.</p> : <div className="alert-preview">{summary.recentAlerts.map((alert) => <div className="alert-preview-row" key={alert.id}><span className="alert-preview-icon">!</span><div><strong>{alert.articleReference} — {alert.articleDesignation}</strong><p>{alert.codeEmplacement} · écart de {alert.ecart > 0 ? '+' : ''}{alert.ecart}</p></div><time>{formatDate(alert.dateSignalement)}</time></div>)}</div>}</article>
        </div>
