...
const StockList = forwardRef(function StockList(_, ref) {
  ...
  return (
    <section className="entity-section" aria-labelledby="stocks-title">
      ...
      {!loading && !error && <><div className="table-search"><label htmlFor="stocks-search">Rechercher</label><input id="stocks-search" type="search" value={searchTerm} onChange={(event) => setSearchTerm(event.target.value)} placeholder="Article ou emplacement…" /></div><div className="table-wrapper"><table><thead><tr><th>Article</th><th>Emplacement</th><th>Quantité disponible</th></tr></thead><tbody>{stocks.length === 0 ? <tr><td colSpan="3" className="empty-cell">Aucun stock disponible.</td></tr> : filteredStocks.length === 0 ? <tr><td colSpan="3" className="empty-cell">Aucun stock ne correspond à cette recherche.</td></tr> : filteredStocks.map((stock) => <tr key={stock.id}><td>{stock.articleReference}</td><td>{stock.codeEmplacement}</td><td>{stock.quantite}</td></tr>)}</tbody></table></div></>}
    </section>
  );
});
